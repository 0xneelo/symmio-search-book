...
Discord is a required demand signal for the Search Book, but it is not yet an imported source corpus. The current repository has an ingestion contract and scraper path, and operator reconciliation marks Discord export/access as provided. The local export file is currently locked by DiscordChatExporter, so the corpus has not yet been regenerated from real messages.
...
The output is `data/discord-corpus.json`. It records import readiness, API scraper readiness, imported-message counts, question clusters, Lafa-answer candidates, missing operator inputs, and seeded topics from `QUESTIONS.md`.
...
When readers ask which Discord answers are authoritative, the answer is still a refusal until the readable export is imported and reviewed. The current source-backed answer is about process: Discord will enter Search Book through a reviewed import path, then question clusters and approved Lafa answers can seed FAQ routes, answer-validation fixtures, gap priority, and living-docs improvements.
