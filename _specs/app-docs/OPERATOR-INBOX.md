...
## Non-Production Coordination Items

These items do not change the canonical production gate boundary above. They are session/tooling issues that block external coordination only; implementation work should continue locally.

### [RESOLVED] #18 — Linear OAuth authorization for SYN-301 status comments
- Need: Restore or authorize Linear OAuth for the current Codex MCP session so `save_comment` can post SYN-301 start/progress/closeout comments. · Why/impact: The attempted SYN-301 start comment failed with `OAuth authorization required`; Linear status visibility is stale unless another authenticated agent/operator mirrors the local checkpoint. · Blocks: Linear comments/status updates for SYN-301 from this session only; it does not block local implementation, verification, commits, or the #11/#4 production gate boundary. · Meanwhile: Continue scoped local implementation in `~/apps/symmio-search-book`, record progress in `_local/agent-worklog.md`, and include the Linear comment failure in final handoff/status. · Resolution (operator): Resolved 2026-07-02: agent-fable-001 (authenticated Linear session) posted the SYN-301 start/closeout comments and mirrored all SYN-304 checkpoint status to Linear, satisfying the visibility need. Codex session-local OAuth does not block the #11/#4 production boundary.
