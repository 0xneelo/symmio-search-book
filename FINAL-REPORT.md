...
The canonical build verifies 24 deterministic build steps, 58 syntax checks, exact-route integrity, FAQ routing, answer chunks, authored-page indexing, requirement coverage, quality-audit gates, readiness-evidence consistency against `data/llm-rag-contract.json`, living-docs summary job presence, and the native sensitive-pattern scan. Live LLM evaluation is not part of every deterministic rebuild because it requires private service credentials and must never print the API key; its latest recorded result is stored in `data/llm-rag-contract.json`.
...
| Deploy preview | Static local prototype exists with `npm run search-book:serve-static`, `npm run search-book:smoke-static`, and combined `npm run search-book:smoke-preview-service` localhost evidence | Production deploy route parked |
