...
The Search Book now has a 500-800 page compendium shape with 794 manifest pages, 799 authored pages, 8 volumes, 104 chapters, and 1,593 reader-routable pages assigned to volumes. The first 323 authored pages are marked `published`; the remaining authored pages stay in candidate/review states. The generated corpus stays inside the requested 500-800 page target while the reader layer includes authored pages plus source companions for traceability.

The authored layer now covers every generated source companion: `data/publication-plan.*` reports 792 source companions queued, 792 covered by authored pages, 0 needing authored coverage, and 475 candidate pages still in the review queue. The remaining candidate review lanes are 0 final-review-ready pages, 475 operator-review pages, 0 source-refresh pages, 0 publication-date-review pages, and 0 editorial-review pages. Source companions remain retrieval and traceability material, not public navigation pages.

The answer-engine front door is implemented as a static prototype plus a shared runtime and standalone SQLite-backed service boundary. The deterministic route map has 798 exact question routes, 32 glossary routes, 2 refusal routes, 2,863 retrieval chunks, and 800 local FAQ entries. The static prototype can be opened directly at `src/search-book/index.html` or served locally with `npm run search-book:serve-static`; `npm run search-book:check-static` verifies local script dependencies, expected `window.SearchBook*` globals, static page links, and public reader-data coverage, while `npm run search-book:smoke-static` proves the Ask front door, exact-page URL, generated data assets, and missing-route 404 behavior over localhost. The service can be run locally with `SEARCH_BOOK_ANSWER_ENGINE_DB=/tmp/search-book-answer-engine.sqlite node src/search-book/scripts/serve-answer-engine.mjs` and connected with `index.html?service=http://127.0.0.1:8787`. The local smoke command `npm run search-book:smoke-service` proves service health, extractive answer persistence, rating persistence, Search Insights, token-gated moderation export, and the internal reviewer gap-summary job against a temporary SQLite database. The combined `npm run search-book:smoke-preview-service` command starts both localhost services, checks the preview-to-service CORS bridge, and verifies service-backed ask, rating, Search Insights, and exact-page URLs without live LLM calls.

The live OpenAI-compatible RAG runtime passed the current recorded SYN-215 eval with `gpt-4.1-mini`: 42/42 total cases, 15/15 adversarial refusals, and 27/27 answer-validation fixtures. Measured usage was 15 calls, 83,256 input tokens, 7,566 output tokens, and an estimated cost of `$0.01702800` at the recorded `gpt-4.1-mini` pricing. This is runtime evidence, not a deployed-service readiness claim.
...
The canonical build verifies 24 deterministic build steps, 59 syntax checks, exact-route integrity, FAQ routing, answer chunks, authored-page indexing, requirement coverage, quality-audit gates, readiness-evidence consistency against `data/llm-rag-contract.json`, static app integrity, living-docs summary job presence, and the native sensitive-pattern scan. Live LLM evaluation is not part of every deterministic rebuild because it requires private service credentials and must never print the API key; its latest recorded result is stored in `data/llm-rag-contract.json`.
...
| Authored layer | 799 authored pages across manifesto, product, rewards, dashboard, protocol, answer-engine, glossary, and compendium sections; 323 published pages; 792/792 source companions covered by authored pages; 0 candidate pages ready for final review before the operator-review lane | Partially published; most pages still need operator/source/deploy review |
...
| Living docs | 12/12 event fixtures, SQLite service, frontend bridge, retention policy, gated moderation export, internal reviewer summary job | Implemented locally; production deployment/admin operations still parked |
