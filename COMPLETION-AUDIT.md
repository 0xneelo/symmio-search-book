...
- Latest local launch drill: generated at `2026-07-02T22:09:51.457Z` with temporary static preview `http://127.0.0.1:44894` and temporary answer-engine service `http://127.0.0.1:45552`.
- Latest local launch drill artifacts: kept under `/tmp/search-book-local-launch-drill-UJbdeN`, with restore-checked backup manifest `/tmp/search-book-local-launch-drill-UJbdeN/backups/latest.manifest.json`.
...
| Deploy or preview docs site | Preview satisfied; production not satisfied | Latest `npm run search-book:drill-local-launch` evidence generated at `2026-07-02T22:09:51.457Z` passed from commit `3058564` against temporary preview `127.0.0.1:44894` and answer service `127.0.0.1:45552` with write-smoke, restore-checked backup, and launch gate evidence. | Public frontend platform/repo/deploy route #4 and production env #11 are still open. |
