...
- `data/llm-rag-contract.json`: live OpenAI `gpt-4.1-mini` eval passed 44/44 on 2026-07-02 (17/17 adversarial, 27/27 answer-validation, financial-advice answered-with-disclaimer policy included).
...
- Latest manual GitHub evidence: launch evidence run `28614271618` and release dry-run run `28614275043` passed from commit `0f9ac05`. Downloaded artifacts at `/tmp/search-book-gh-manual-launch-28614271618` and `/tmp/search-book-gh-manual-release-28614275043` passed strict summary validation with `npm run search-book:check-launch-evidence-packet -- --packet /tmp/search-book-gh-manual-launch-28614271618/search-book-launch-evidence/launch-evidence.json --require-summary` and `npm run search-book:check-release-dry-run-packet -- --packet /tmp/search-book-gh-manual-release-28614275043/search-book-release-dry-run/release-dry-run.json --require-summary`; the nested release launch packet also passed checked validation. Both summary artifacts carry repository commit `0f9ac05`, dirty `false`, `Discord editorial queue data | passed (160 routed / 91 page-fit / 13 refusals; ready: true)`, `Discord editorial disposition | ready true (keep-copy 91/91; keep-refusal 13/13; copy changes 0; promoted 0)`, `Discord reviewer workflow | ready ready (4 phases; page-fit 91; refusals 13; copy changes allowed 0; exact promotions allowed 0)`, `Open operator Linear tasks | #4=SYN-285, #11=SYN-281`, and `Secrets printed | false`; release evidence also reports static artifact integrity `passed`, 1,652 files, 53,997,387 bytes, sensitive matches `0`, source ingestion `17/17`, `productionReadinessPacket:passed`, spec reconciliation `passed`, 93 syntax checks, and only #11/#4 open. The launch, release, and static-artifact check-run annotations were all empty (`[]`) for jobs `84854057367`, `84854070383`, and `84854060400`, confirming the manual evidence lane has no workflow warnings.
- Latest platform-neutral static artifact evidence: manual `Search Book Static Artifact` workflow run `28614272577` passed from commit `0f9ac05`. Downloaded artifact `/tmp/search-book-gh-static-artifact-28614272577/search-book-static-site` passed `npm run search-book:check-static-artifact-packet`, `npm run search-book:smoke-static`, and `npm run search-book:smoke-preview-service` with 1,652 copied files, 53,997,387 bytes, integrity `passed`, `valuesPrinted:false`, 0 sensitive-pattern matches, source ingestion `17/17`, Discord route coverage `91/91`, service-backed ask/rating/page-feedback persistence, and only #11/#4 open. The release bundle static site at `/tmp/search-book-gh-manual-release-28614275043/search-book-release-dry-run/static-site` also passed checked static packet validation plus static and preview-service smokes.
...
| Build answer-engine front door with exact page routing and citations | Satisfied locally | 890/890 exact routes, 32/32 glossary routes, 2/2 refusal routes, 2,884 chunks; local preview/service and staging launch drill passed; live LLM eval passed 44/44. | Production model-backed service still depends on #11 and deploy route #4. |
| Refuse unsupported, unsafe, secret, financial-advice, and out-of-scope questions | Satisfied in current eval contract (financial-advice lane operator-amended 2026-07-02 to answer-with-disclaimer) | `data/llm-rag-contract.json` and `data/answer-validation-report.json` show 17/17 adversarial and 27/27 answer-validation fixtures passing, including the G-008 Vibe covered-call/vault-backed inventory source-boundary refusal. Financial-advice questions now return grounded answered-with-disclaimer responses (`advisory: not-financial-advice`, never a specific action recommendation) per the operator decision, covered by blunt plus paraphrase fixtures; they still refuse when no grounded context exists. | Re-run live eval after production deployment wiring. |
