...
- `data/llm-rag-contract.json`: live OpenAI `gpt-4.1-mini` eval passed 46/46 on 2026-07-02 (19/19 adversarial, 27/27 answer-validation, financial-advice answered-with-disclaimer policy included).
...
- Latest local launch drill: generated at `2026-07-02T17:24:05.164Z` with temporary static preview `http://127.0.0.1:45642` and temporary answer-engine service `http://127.0.0.1:45490`.
- Latest local launch drill artifacts: kept under `/tmp/search-book-local-launch-drill-Tt3cxu`, with restore-checked backup manifest `/tmp/search-book-local-launch-drill-Tt3cxu/backups/latest.manifest.json`.
- Latest local launch drill result: passed with deployment write-smoke answer `answered`, rating `recorded`, pageFeedback `recorded`, 2 citations, backup `passed`, restore-check `passed`, launch readiness 15/15 staging checks, 0 failures, 0 warnings, fresh verify 26 build steps / 93 syntax checks, `valuesPrinted:false`, and no LLM API key loaded.
...
| Deploy or preview docs site | Preview satisfied; production not satisfied | Latest `npm run search-book:drill-local-launch` evidence generated at `2026-07-02T17:24:05.164Z` passed against temporary preview `127.0.0.1:45642` and answer service `127.0.0.1:45490` with write-smoke, restore-checked backup, and launch gate evidence. | Public frontend platform/repo/deploy route #4 and production env #11 are still open. |
