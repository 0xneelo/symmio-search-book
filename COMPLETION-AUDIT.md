...
| Build living-docs loop: track questions, rate answers, surface gaps | Satisfied locally, not production-ready | SQLite service persists questions/ratings/gaps/cache; Search Insights, moderation export, metrics export, retention, reviewer summary, backup/restore-check, and CI-safe backup-restore evidence are implemented; staged launch drill recorded answer/rating persistence. | Production deploy, reviewer owner/cadence, backup storage, moderation/metrics access, and #11/#4 remain. |
...
| No secrets leaked | Satisfied by current checks | Local env is not printed; launch/preflight outputs report booleans only; CI is no-secret and covered by `npm run search-book:check-github-workflows`; sensitive scan runs in `npm run search-book:verify`. | Continue loading LLM creds only via env-file and never commit local DB/env artifacts. |
