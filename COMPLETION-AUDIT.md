...
- Latest local launch drill: generated at `2026-07-02T11:17:03.738Z` with temporary static preview `http://127.0.0.1:46074` and temporary answer-engine service `http://127.0.0.1:45748`.
- Latest local launch drill artifacts: kept under `/tmp/search-book-local-launch-drill-jXweVE`, with restore-checked backup manifest `/tmp/search-book-local-launch-drill-jXweVE/backups/latest.manifest.json`.
...
| Deploy or preview docs site | Preview satisfied; production not satisfied | Latest `npm run search-book:drill-local-launch` evidence generated at `2026-07-02T11:17:03.738Z` passed against temporary preview `127.0.0.1:46074` and answer service `127.0.0.1:45748` with write-smoke, restore-checked backup, and launch gate evidence. | Public frontend platform/repo/deploy route #4 and production env #11 are still open. |
