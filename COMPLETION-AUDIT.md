...
- `data/discord-review-routing.json`: sanitized Discord/Lafa routing summary ready with 160 routed items, 91/91 page-fit groups covered by public route aliases, source-backed triage 91/91 page-fit groups, public-copy ready 91/91 page-fit groups, public-copy review required 0/91 page-fit groups, refusal policy ready 13/13 refusal items, refusal policy review required 0/13 refusal items, 0 single-route groups remaining, and no raw Discord/Lafa/source answer text included.
- `DISCORD-EDITORIAL-QUEUE.md` and `data/discord-editorial-queue.*`: no-raw reviewer queues generated from the sanitized routing summary, listing page-fit and refusal-review work with item ids, page ids, source keys, counts, automated triage status, public-copy readiness status, refusal policy status, refusal reasons, and automated disposition only; current disposition is reviewer-handoff ready with 91/91 page-fit groups keeping existing source-backed public copy, 13/13 refusal items keeping refusal policy, 0 public-copy changes proposed, and 0 exact Discord/Lafa statements promoted. The queue grounding evidence now proves 91/91 page-fit groups resolve to published authored pages, carry queued source keys on the page and in `data/source-catalog.json`, include source URLs plus `## Sources`, and have public route counts matching `data/question-routes.json`, with 0 grounding failures. `npm run search-book:check-discord-review-artifacts` validates both against the JSON summary, and the latest current-head closeout validated the raw/internal `/tmp/search-book-discord-review-closeout-5a39841/discord-review-queue.json` plus sanitized `/tmp/search-book-discord-routing-closeout-5a39841/discord-review-routing.json` packets with 80 question-review items, 80 Lafa-review items, 160 routed items, `rawKeyHits:0`, and `sampleLeaks:0`.
- `npm run search-book:check-discord-refusals`: public-safe runtime guardrail check passes with one Lafa-founder answer carrying a `discord-lafa` citation and one non-Lafa community-quote refusal carrying `refusalReason:"discord-corpus-review-required"`, `gapId:"G-001"`, zero citations, zero answer bytes, no primary page, and no LLM credentials loaded.
...
- `data/question-routes.json`: 890 exact routes.
- `data/faq.json`: 892 FAQ entries.
...
- `data/llm-rag-contract.json`: live OpenAI `gpt-4.1-mini` eval passed 47/47 on 2026-07-02 (20/20 adversarial, 27/27 answer-validation, financial-advice answered-with-disclaimer policy included).
...
- Latest local launch drill: generated at `2026-07-02T21:11:07.925Z` with temporary static preview `http://127.0.0.1:46052` and temporary answer-engine service `http://127.0.0.1:44744`.
- Latest local launch drill artifacts: kept under `/tmp/search-book-local-launch-drill-C5hGYv`, with restore-checked backup manifest `/tmp/search-book-local-launch-drill-C5hGYv/backups/latest.manifest.json`.
- Latest local launch drill result: passed with deployment write-smoke answer `answered`, rating `recorded`, pageFeedback `recorded`, 2 citations, backup `passed`, restore-check `passed`, launch readiness 15/15 staging checks, 0 failures, 0 warnings, fresh verify 26 build steps / 95 syntax checks, `valuesPrinted:false`, and no LLM API key loaded.
- Latest manual GitHub evidence: launch evidence run `28622312449` and release dry-run run `28622310504` passed from commit `110c684`. Downloaded artifacts at `/tmp/search-book-gh-manual-launch-28622312449` and `/tmp/search-book-gh-manual-release-28622310504` passed strict summary validation with `npm run search-book:check-launch-evidence-packet -- --packet /tmp/search-book-gh-manual-launch-28622312449/search-book-launch-evidence/launch-evidence.json --require-summary` and `npm run search-book:check-release-dry-run-packet -- --packet /tmp/search-book-gh-manual-release-28622310504/search-book-release-dry-run/release-dry-run.json --require-summary`; the nested release launch packet also passed checked validation. Both summary artifacts carry repository commit `110c684`, dirty `false`, `Discord editorial queue data | passed (160 routed / 91 page-fit / 13 refusals; ready: true)`, `Discord editorial disposition | ready true (keep-copy 91/91; keep-refusal 13/13; copy changes 0; promoted 0)`, `Discord reviewer workflow | ready ready (4 phases; page-fit 91; refusals 13; copy changes allowed 0; exact promotions allowed 0)`, `Open operator Linear tasks | #4=SYN-285, #11=SYN-281`, and `Secrets printed | false`; release evidence also reports static artifact integrity `passed`, 1,654 files, 54,402,594 bytes, sensitive matches `0`, source ingestion `17/17`, `productionReadinessPacket:passed`, spec reconciliation `passed`, 95 syntax checks, and only #11/#4 open. The launch, release, and static-artifact check-run annotations were all empty (`[]`) for jobs `84880761974`, `84880755772`, and `84880755457`, confirming the manual evidence lane has no workflow warnings.
- Latest platform-neutral static artifact evidence: manual `Search Book Static Artifact` workflow run `28622310287` passed from commit `110c684`. Downloaded artifact `/tmp/search-book-gh-static-artifact-28622310287/search-book-static-site` passed `npm run search-book:check-static-artifact-packet`, `npm run search-book:smoke-static`, and `npm run search-book:smoke-preview-service` with 1,654 copied files, 54,402,594 bytes, integrity `passed`, `valuesPrinted:false`, 0 sensitive-pattern matches, source ingestion `17/17`, Discord route coverage `91/91`, service-backed ask/rating/page-feedback persistence, and only #11/#4 open. The release bundle static site at `/tmp/search-book-gh-manual-release-28622310504/search-book-release-dry-run/static-site` also passed checked static packet validation plus static and preview-service smokes.
...
| Mine Symmio Discord and Lafa answers into FAQ/answer-engine seed | Satisfied for seed corpus; exact-claim review remains | `scripts/build-discord-corpus.mjs` imported the real export in internal-only mode; `data/discord-corpus.json` reports 5,000 messages, 723 question clusters, 837 configured Lafa candidates, and `corpusReady:true`; `DISCORD-EDITORIAL-QUEUE.md` gives reviewers a no-raw page-fit/refusal queue from the sanitized routing summary with source-backed triage 91/91 page-fit groups, public-copy ready 91/91 page-fit groups, public-copy review required 0/91 page-fit groups, refusal policy ready 13/13 refusal items, refusal policy review required 0/13 refusal items, and disposition evidence that proposes 0 public-copy changes and promotes 0 exact Discord/Lafa statements. | Review local raw packets before any future public paraphrase; current committed disposition keeps existing public copy/refusals. |
...
| Build answer-engine front door with exact page routing and citations | Satisfied locally | 890/890 exact routes, 32/32 glossary routes, 2/2 refusal routes, 2,884 chunks; local preview/service and staging launch drill passed; live LLM eval passed 47/47. | Production model-backed service still depends on #11 and deploy route #4. |
| Refuse unsupported, unsafe, secret, financial-advice, and out-of-scope questions | Satisfied in current eval contract (financial-advice lane operator-amended 2026-07-02 to answer-with-disclaimer) | `data/llm-rag-contract.json` and `data/answer-validation-report.json` show 20/20 adversarial and 27/27 answer-validation fixtures passing, including the G-008 Vibe covered-call/vault-backed inventory source-boundary refusal. Financial-advice questions now return grounded answered-with-disclaimer responses (`advisory: not-financial-advice`, never a specific action recommendation) per the operator decision, covered by blunt plus paraphrase fixtures; they still refuse when no grounded context exists. | Re-run live eval after production deployment wiring. |
...
| Deploy or preview docs site | Preview satisfied; production not satisfied | Latest `npm run search-book:drill-local-launch` evidence generated at `2026-07-02T21:11:07.925Z` passed from commit `714cae0` against temporary preview `127.0.0.1:46052` and answer service `127.0.0.1:44744` with write-smoke, restore-checked backup, and launch gate evidence. | Public frontend platform/repo/deploy route #4 and production env #11 are still open. |
