...
- `DISCORD-EDITORIAL-QUEUE.md` and `data/discord-editorial-queue.*`: no-raw reviewer queues generated from the sanitized routing summary, listing page-fit and refusal-review work with item ids, page ids, source keys, counts, automated triage status, public-copy readiness status, refusal policy status, and refusal reasons only; `npm run search-book:check-discord-review-artifacts` validates both against the JSON summary.
