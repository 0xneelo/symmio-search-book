...
- Latest manual GitHub evidence: launch evidence run `28621010906` and release dry-run run `28621012744` passed from commit `ad2c695`. Downloaded artifacts at `/tmp/search-book-gh-manual-launch-28621010906` and `/tmp/search-book-gh-manual-release-28621012744` passed strict summary validation with `npm run search-book:check-launch-evidence-packet -- --packet /tmp/search-book-gh-manual-launch-28621010906/search-book-launch-evidence/launch-evidence.json --require-summary` and `npm run search-book:check-release-dry-run-packet -- --packet /tmp/search-book-gh-manual-release-28621012744/search-book-release-dry-run/release-dry-run.json --require-summary`; the nested release launch packet also passed checked validation. Both summary artifacts carry repository commit `ad2c695`, dirty `false`, `Discord editorial queue data | passed (160 routed / 91 page-fit / 13 refusals; ready: true)`, `Discord editorial disposition | ready true (keep-copy 91/91; keep-refusal 13/13; copy changes 0; promoted 0)`, `Discord reviewer workflow | ready ready (4 phases; page-fit 91; refusals 13; copy changes allowed 0; exact promotions allowed 0)`, `Open operator Linear tasks | #4=SYN-285, #11=SYN-281`, and `Secrets printed | false`; release evidence also reports static artifact integrity `passed`, 1,654 files, 54,402,594 bytes, sensitive matches `0`, source ingestion `17/17`, `productionReadinessPacket:passed`, spec reconciliation `passed`, 93 syntax checks, and only #11/#4 open. The launch, release, and static-artifact check-run annotations were all empty (`[]`) for jobs `84876422834`, `84876427764`, and `84876433854`, confirming the manual evidence lane has no workflow warnings.
- Latest platform-neutral static artifact evidence: manual `Search Book Static Artifact` workflow run `28621014214` passed from commit `ad2c695`. Downloaded artifact `/tmp/search-book-gh-static-artifact-28621014214/search-book-static-site` passed `npm run search-book:check-static-artifact-packet`, `npm run search-book:smoke-static`, and `npm run search-book:smoke-preview-service` with 1,654 copied files, 54,402,594 bytes, integrity `passed`, `valuesPrinted:false`, 0 sensitive-pattern matches, source ingestion `17/17`, Discord route coverage `91/91`, service-backed ask/rating/page-feedback persistence, and only #11/#4 open. The release bundle static site at `/tmp/search-book-gh-manual-release-28621012744/search-book-release-dry-run/static-site` also passed checked static packet validation plus static and preview-service smokes.
