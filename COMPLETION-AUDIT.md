...
- Latest local launch drill: generated at `2026-07-02T04:47:29.041Z` with temporary static preview `http://127.0.0.1:46268` and temporary answer-engine service `http://127.0.0.1:45754`.
- Latest local launch drill artifacts: kept under `/tmp/search-book-local-launch-drill-aUDtU5`, with restore-checked backup manifest `/tmp/search-book-local-launch-drill-aUDtU5/backups/latest.manifest.json`.
...
- Latest manual GitHub evidence: launch evidence run `28566355604` and release dry-run run `28566356824` passed from commit `1c6589d`. Downloaded artifacts at `/tmp/search-book-gh-manual-launch-28566355604` and `/tmp/search-book-gh-manual-release-28566356824` passed strict summary validation with `npm run search-book:check-launch-evidence-packet -- --packet /tmp/search-book-gh-manual-launch-28566355604/search-book-launch-evidence/launch-evidence.json --require-summary` and `npm run search-book:check-release-dry-run-packet -- --packet /tmp/search-book-gh-manual-release-28566356824/search-book-release-dry-run/release-dry-run.json --require-summary`; the nested release launch packet also passed checked validation without requiring a separate nested summary artifact. Both summary artifacts carry repository commit `1c6589d`, dirty `false`, `Discord editorial queue data | passed`, `Discord editorial disposition | ready true (keep-copy 19/19; keep-refusal 2/2; copy changes 0; promoted 0)`, `Discord reviewer workflow | ready ready (4 phases; page-fit 19; refusals 2; copy changes allowed 0; exact promotions allowed 0)`, `Open operator Linear tasks | #4=SYN-285, #11=SYN-281`, and `Secrets printed | false`; release evidence also reports static artifact integrity `passed`, 1,652 files, 53,476,055 bytes, sensitive matches `0`, source ingestion `17/17`, `productionReadinessPacket:passed`, spec reconciliation `passed`, 93 syntax checks, and only #11/#4 open. The launch, release, and static-artifact check-run annotations were all empty (`[]`) for jobs `84694362836`, `84694366255`, and `84694364416`, confirming the manual evidence lane has no workflow warnings.
- Latest platform-neutral static artifact evidence: manual `Search Book Static Artifact` workflow run `28566356203` passed from commit `1c6589d`. Downloaded artifact `/tmp/search-book-gh-static-artifact-28566356203/search-book-static-site` passed `npm run search-book:check-static-artifact-packet`, `npm run search-book:smoke-static`, and `npm run search-book:smoke-preview-service` with 1,652 copied files, 53,476,055 bytes, integrity `passed`, `valuesPrinted:false`, 0 sensitive-pattern matches, source ingestion `17/17`, Discord route coverage `19/19`, service-backed ask/rating/page-feedback persistence, and only #11/#4 open.
...
| Deploy or preview docs site | Preview satisfied; production not satisfied | Latest `npm run search-book:drill-local-launch` evidence generated at `2026-07-02T04:47:29.041Z` passed against temporary preview `127.0.0.1:46268` and answer service `127.0.0.1:45754` with write-smoke, restore-checked backup, and launch gate evidence. | Public frontend platform/repo/deploy route #4 and production env #11 are still open. |
