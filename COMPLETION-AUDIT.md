...
- Latest local launch drill: generated at `2026-07-02T09:17:18.217Z` with temporary static preview `http://127.0.0.1:46248` and temporary answer-engine service `http://127.0.0.1:45502`.
- Latest local launch drill artifacts: kept under `/tmp/search-book-local-launch-drill-YRrHv9`, with restore-checked backup manifest `/tmp/search-book-local-launch-drill-YRrHv9/backups/latest.manifest.json`.
...
| Deploy or preview docs site | Preview satisfied; production not satisfied | Latest `npm run search-book:drill-local-launch` evidence generated at `2026-07-02T09:17:18.217Z` passed against temporary preview `127.0.0.1:46248` and answer service `127.0.0.1:45502` with write-smoke, restore-checked backup, and launch gate evidence. | Public frontend platform/repo/deploy route #4 and production env #11 are still open. |
