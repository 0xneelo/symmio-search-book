...
The answer-engine evidence reports 890 exact public question routes, 892 FAQ entries, 890/890 exact-route tests, 32/32 glossary route tests, 2/2 refusal tests, and 27/27 answer-validation fixtures. The live OpenAI `gpt-4.1-mini` eval is 44/44 total, with 17/17 adversarial cases and 27/27 answer-validation cases (the 2026-07-02 financial-advice policy adds two live-only answered-with-disclaimer adversarial cases excluded from the static answer-validation mirror). Local `.secrets/search-book.env` is complete for local LLM work and must not be printed; OPERATOR-INBOX #11 is only the production VPS service env install at `/etc/symmio-search-book/search-book.env`.
...
## D-197: Finish Referral Section 17 Exact Coverage

**Decision:** Add authored Volume 06 rewards-reference pages for the final uncovered Referral Program headings: metrics decision use, rewards-pack guardrails, rewards-pack rollout sequencing, tokenized-points strategic upside, and the referral source table of contents. Map existing LP/category, security, metrics, tokenized-points, and traceability pages to their exact generated headings.
