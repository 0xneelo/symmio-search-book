...
- Local preview evidence: latest local launch drill passed against temporary preview `127.0.0.1:45992` plus service `127.0.0.1:46102`, with write-smoke, restore-check, and 15/15 staging launch checks from generatedAt `2026-07-02T12:08:36.956Z`
- Latest strict manual launch/release evidence: launch run `28587586501` and release run `28587586928` passed from commit `c023299`; downloaded artifacts under `/tmp/search-book-gh-manual-launch-28587586501` and `/tmp/search-book-gh-manual-release-28587586928` passed strict summary validation without printing secrets, and check-run annotations were empty (`[]`)
