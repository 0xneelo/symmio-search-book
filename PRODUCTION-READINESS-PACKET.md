...
- Local preview evidence: latest local launch drill passed against temporary preview `127.0.0.1:45812` plus service `127.0.0.1:45990`, with write-smoke, restore-check, and 15/15 staging launch checks from generatedAt `2026-07-02T00:24:37.711Z`
- Latest strict manual launch/release evidence: launch run `28557496582` and release run `28557496079` passed from commit `bd1cae7`; downloaded artifacts under `/tmp/search-book-gh-manual-launch-28557496582` and `/tmp/search-book-gh-manual-release-28557496079` passed strict summary validation without printing secrets, and check-run annotations were empty (`[]`)
...
- Current deterministic evidence: 850 exact routes, 2,884 chunks, 801 authored pages,
