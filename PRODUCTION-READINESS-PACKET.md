...
- Local preview evidence: latest local launch drill passed against temporary preview `127.0.0.1:45322` plus service `127.0.0.1:46376`, with write-smoke, restore-check, and 15/15 staging launch checks from generatedAt `2026-07-02T10:16:35.353Z`
- Latest strict manual launch/release evidence: launch run `28583096076` and release run `28583097424` passed from commit `df856b5`; downloaded artifacts under `/tmp/search-book-gh-manual-launch-28583096076` and `/tmp/search-book-gh-manual-release-28583097424` passed strict summary validation without printing secrets, and check-run annotations were empty (`[]`)
