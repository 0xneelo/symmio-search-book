...
- Local preview evidence: latest local launch drill passed against temporary preview `127.0.0.1:45702` plus service `127.0.0.1:45972`, with write-smoke, restore-check, and 15/15 staging launch checks from generatedAt `2026-07-02T07:14:18.874Z`
- Latest strict manual launch/release evidence: launch run `28571262091` and release run `28571262743` passed from commit `086ae7a`; downloaded artifacts under `/tmp/search-book-gh-manual-launch-28571262091` and `/tmp/search-book-gh-manual-release-28571262743` passed strict summary validation without printing secrets, and check-run annotations were empty (`[]`)
