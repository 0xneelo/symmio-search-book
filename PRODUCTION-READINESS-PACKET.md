...
- Local preview evidence: latest local launch drill passed against temporary preview `127.0.0.1:46288` plus service `127.0.0.1:46216`, with write-smoke, restore-check, and 15/15 staging launch checks from generatedAt `2026-07-01T23:19:57.093Z`
- Latest strict manual launch/release evidence: launch run `28554912092` and release run `28554912403` passed from commit `2439dfe`; downloaded artifacts under `/tmp/search-book-gh-manual-launch-28554912092` and `/tmp/search-book-gh-manual-release-28554912403` passed strict summary validation without printing secrets, and check-run annotations were empty (`[]`)
