...
- Local preview evidence: latest local launch drill passed against temporary preview `127.0.0.1:46482` plus service `127.0.0.1:45006`, with write-smoke, restore-check, and 15/15 staging launch checks from generatedAt `2026-07-02T02:20:15.743Z`
- Latest strict manual launch/release evidence: launch run `28561587749` and release run `28561587711` passed from commit `b1078f2`; downloaded artifacts under `/tmp/search-book-gh-manual-launch-28561587749` and `/tmp/search-book-gh-manual-release-28561587711` passed strict summary validation without printing secrets, and check-run annotations were empty (`[]`)
