...
function runStatusEvidenceCheck(env, dryRun) {
  const step = {
    ...
    command: process.execPath,
    args: [path.join(searchBookRoot, "scripts", "check-status-evidence.mjs")],
  };
  if (dryRun) {
    console.log(commandLine(step));
    return { dryRun: true };
...
function runDeployTemplatesCheck(env, dryRun) {
  const step = {
    id: "check-deploy-templates",
    command: process.execPath,
    args: [path.join(searchBookRoot, "scripts", "check-deploy-templates.mjs")],
  };
  if (dryRun) {
    console.log(commandLine(step));
    return { dryRun: true };
...
function runInvariants() {
  ...
  assert(
    living.retentionPolicyImplemented &&
      living.moderationExportImplemented &&
      living.metricsExportImplemented &&
      living.corsPolicyImplemented &&
      living.pageFeedbackServiceImplemented &&
      living.gapSummaryJobImplemented &&
      living.backupRestoreImplemented &&
      living.productionPreflightImplemented &&
      !living.livingDocsProductionReady,
    "living-docs production boundary is unexpected",
  );
