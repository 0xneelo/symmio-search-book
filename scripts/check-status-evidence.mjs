...
function expectedChecks(evidence) {
  ...
  return {
    ...
    "DECISIONS.md": [
      ...
      { id: "live-llm-eval", allOf: [`live ${evidence.liveEvalProvider} \`${evidence.liveEvalModel}\` eval`, `${evidence.liveEvalTotal} total`, `${evidence.liveEvalAdversarial} adversarial cases`, `${evidence.liveEvalAnswerValidation} answer-validation cases`, "Local `.secrets/search-book.env` is complete", "/etc/symmio-search-book/search-book.env"] },
      { id: "approved-revenue-referral", allOf: ["networkVolume × platformFeeRate × referrerPlatformShare", "0.05%", "5 bps", "30% referrer platform share", "Phase B economics remain out of scope for v1", "15 levels", "additive so it never lowers"] },
    ...
    "STYLEGUIDE.md": [
      ...
      { id: "answer-engine-evidence", allOf: [`${evidence.exactRoutes} exact public question routes`, `${evidence.faqEntries} FAQ entries`, `${evidence.exactRouteTests} exact-route tests`, `${evidence.glossaryRouteTests} glossary route tests`, `${evidence.refusalTests} refusal tests`, `${evidence.answerValidationFixtures} answer-validation fixtures`] },
      { id: "live-llm-eval", allOf: [`live ${evidence.liveEvalProvider} \`${evidence.liveEvalModel}\` eval passes ${evidence.liveEvalTotal}`, `${evidence.liveEvalAdversarial} adversarial cases`, `${evidence.liveEvalAnswerValidation} answer-validation cases`] },
      ...
      { id: "approved-revenue-referral", allOf: ["networkVolume x platformFeeRate x referrerPlatformShare", "0.05%", "5 bps", "30% referrer platform share", "Phase B economics are out of scope for v1", "15 levels", "additive and never lowers"] },
      { id: "amfq-terminology", allOf: ["AMFQ/aMFQ", "Automated Market for Quotes", "legacy Vibe name for Intents", "Use `Intent` in current docs"] },
      { id: "source-boundaries", allOf: ["Notion: registered as `vibe-trading-notion` with a paraphrase-only public-use boundary", "SSHE: v1 boundary is SuperFlow/SHE OpenAPI plus Symmio Foundation Meta-Solvers/Clearing Layers", "Symmio whitepaper history: original/pre-v0.8 recovery is out of scope for v1", "Add Token Info: use the official Markdown source", "Do not quote raw Discord/Lafa text"] },
    ...
    "QUESTIONS.md": [
      ...
      { id: "discord-boundary", allOf: ["Discord/Lafa import is internal-only", `${evidence.discordMessages} imported messages`, `${evidence.discordClusters} question clusters`, `${evidence.discordLafaCandidates} configured Lafa answer candidates`, "raw non-Lafa Discord community text stays out of public answers", "specific non-Lafa Discord community claims remain non-public until editorial review"] },
    ...
    "DISCORD-EDITORIAL-QUEUE.md": [
      ...
      { id: "privacy-boundary", allOf: ["Raw Discord text included: `false`", "Source answer text included: `false`", "Values printed: `false`", "do not quote Discord or Lafa", "Promote only primary-source paraphrases"] },
