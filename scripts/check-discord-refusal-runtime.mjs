...
const runtimeScript = path.join(searchBookRoot, "scripts", "run-llm-rag-answer.mjs");
...
function validateRuntimeProbe(probe, checks) {
  ...
  addCheck(
    ...
    { id: probe.id, stdoutBytes: probe.stdoutBytes, stderrBytes: probe.stderrBytes },
  );
  addCheck(
    checks,
...
const probes = runtimeProbes.map(runProbe);
