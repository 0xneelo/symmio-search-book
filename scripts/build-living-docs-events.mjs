...
function parseArgs(argv) {
  ...
  for (let index = 0; index < argv.length; index += 1) {
    ...
    if (arg === "--out-json") args.outJson = argv[++index];
      ...
      console.log("Usage: node scripts/build-living-docs-events.mjs [--out-json path] [--out-js path]");
      process.exit(0);
    } else {
...
const payload = {
  ...
  requiredRuntimeBehaviors: [
    ...
    "Create an asked-fact-not-in-corpus gap when LLM synthesis answers that supplied context does not provide a requested fact.",
