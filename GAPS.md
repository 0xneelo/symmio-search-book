...
The candidate review queue now includes the remaining `76` candidate pages after the first `722` source/prose/route/current-source/operator-boundary-reviewed pages moved to `published`. Its launch lanes are `0` final-review-ready pages, `76` operator-review pages, `0` source-refresh pages, `0` publication-date-review pages, and `0` editorial-review pages. The product-reference, competitive-context, compendium singleton, Volume 02, Volume 03, Volume 04, Volume 05, protocol-reference, and manifesto lanes have no remaining candidate pages; rewards-referrals candidates remain `76`, all in Volume 06.
