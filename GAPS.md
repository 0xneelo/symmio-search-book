...
The latest Volume 04 publication batch completes the Funding Rate Model candidate track: magnet/gradient analogies, normal/stress utilization examples, phase and counterparty share, rate regimes, risk inversion, risk-signal maps, risk/volatility parameters, state variables, pricing instruments, token-inventory mode, trader PnL phase exposure, trader UX versus LP efficiency, two-mode utilization switching, unified-objective preview, Vibe-versus-Uniswap LP risk, worked-example reading guidance, gradient-flow market balancing, liquidations as inventory reallocation, and inventory/insurance utilization modes. It keeps live regime thresholds, utilization bands, risk/volatility parameters, phase/counterparty-share values, ADL exposure settings, liquidation behavior, pricing instruments, inventory/insurance switch mechanics, trader PnL exposure, LP yield expectations, Uniswap/Vibe comparisons, and legal/risk/accounting-sensitive claims under source/operator/product/risk/legal/accounting review. Previous Volume 04 funding batches cover funding formula/state/control pages, funding-objective and insurance coverage, funding-controls pages, and risk-premium/token-margin pages.

The candidate review queue now includes the remaining `259` candidate pages after the first `539` source/prose/route/current-source/operator-boundary-reviewed pages moved to `published`. Its launch lanes are `0` final-review-ready pages, `259` operator-review pages, `0` source-refresh pages, `0` publication-date-review pages, and `0` editorial-review pages. The product-reference, competitive-context, compendium singleton, Volume 02, Volume 03, and Volume 04 Funding Rate Model lanes have no remaining candidate pages; protocol-reference candidates are down to `96`, rewards-referrals candidates are down to `84`, manifesto candidates are down to `79`, and the remaining volume lanes are Volume 04 (`67`), Volume 05 (`108`), and Volume 06 (`84`).
