...
The latest Volume 05 publication batch publishes the funding-defense and protocol-operations cluster covering conditional global insurance allocation, conservative launch collateralization, discontinuous-market guardrails, external USDC LP risk-premium mismatch, ADL priority/target/trigger handling, bell-curve transfer logic, defense activation/cost ordering, exposure loss estimates, insurance spend caps, hedge-cost coverage, local/global insurance, solver token-inventory defense, netting, defense budgets, and surplus/shortfall accounting. It keeps live insurance allocation, launch collateralization, ADL settings, bell-curve thresholds, hedge-cost coverage, defense budgets, surplus/shortfall accounting, and legal/risk/accounting/security-sensitive advice under primary-source/operator/product/risk/legal/security review.

The candidate review queue now includes the remaining `168` candidate pages after the first `630` source/prose/route/current-source/operator-boundary-reviewed pages moved to `published`. Its launch lanes are `0` final-review-ready pages, `168` operator-review pages, `0` source-refresh pages, `0` publication-date-review pages, and `0` editorial-review pages. The product-reference, competitive-context, compendium singleton, Volume 02, Volume 03, and Volume 04 lanes have no remaining candidate pages; protocol-reference candidates are down to `71`, rewards-referrals candidates remain `84`, manifesto candidates remain `13`, and the remaining volume lanes are Volume 05 (`84`) and Volume 06 (`84`).
