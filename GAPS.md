...
The latest Volume 05 publication batches publish the funding-defense and solver-risk operations clusters. The funding-defense pass covers conditional global insurance allocation, conservative launch collateralization, discontinuous-market guardrails, external USDC LP risk-premium mismatch, ADL priority/target/trigger handling, bell-curve transfer logic, defense activation/cost ordering, exposure loss estimates, insurance spend caps, hedge-cost coverage, local/global insurance, solver token-inventory defense, netting, defense budgets, and surplus/shortfall accounting. The solver-risk pass covers internal inventory, internal netting, liquidity-collapse freeze logic, protective posture, first-loss/loss-waterfall handling, LP vault exposure, market-tier limits, netting-state risk transfer, protocol-owned solver depletion, residual-counterparty spread/hedge-first execution, RFQ/last-look controls, strict/soft liquidation modes, CVA buffers, solver default/continuity, hedging failure/resource buffers, strategic unhedged exposure, tail-event profit caps, trader compensation continuity, and VibeCaps hedge-first requirements. These pages keep live insurance allocation, launch collateralization, ADL settings, bell-curve thresholds, hedge-cost coverage, defense budgets, netting rules, solver liquidation/default behavior, compensation, VibeCaps inventory/hedge requirements, and legal/risk/accounting/security-sensitive advice under primary-source/operator/product/risk/legal/security review.

The candidate review queue now includes the remaining `143` candidate pages after the first `655` source/prose/route/current-source/operator-boundary-reviewed pages moved to `published`. Its launch lanes are `0` final-review-ready pages, `143` operator-review pages, `0` source-refresh pages, `0` publication-date-review pages, and `0` editorial-review pages. The product-reference, competitive-context, compendium singleton, Volume 02, Volume 03, and Volume 04 lanes have no remaining candidate pages; protocol-reference candidates are down to `46`, rewards-referrals candidates remain `84`, manifesto candidates remain `13`, and the remaining volume lanes are Volume 05 (`59`) and Volume 06 (`84`).
