...
The latest Volume 04 publication batch promotes the first Percolator/token-margined risk-foundations cluster: inverse payoff, linear-versus-hyperbolic payout, 1x leverage ceilings, gross OI collateral lockup, JIT liquidity duration mismatch, passive matcher vulnerability, oracle-as-reference versus solver quotes, inverse-product historical retreat, Futureswap toxic-arbitrage precedent, industry migration toward stable settlement, and Percolator's balance-sheet, trust-boundary, formal-verification, one-market-one-slab, hybrid risk/execution, source-map, and matcher-pluggability pages. It keeps live Percolator/SOV parameters, deployed-program status, oracle/matcher behavior, market/liquidity claims, inverse-settlement math, historical venue facts, token payout guarantees, formal-verification guarantees, and risk/legal/security-sensitive claims under primary-source/operator/product/risk/legal/security review. The previous Volume 04 publication batch completed the USDC-vs-token-margined candidate track; earlier Volume 04 funding batches completed the Funding Rate Model candidate track and cover funding formula/state/control pages, funding-objective and insurance coverage, funding-controls pages, and risk-premium/token-margin pages.

The candidate review queue now includes the remaining `220` candidate pages after the first `578` source/prose/route/current-source/operator-boundary-reviewed pages moved to `published`. Its launch lanes are `0` final-review-ready pages, `220` operator-review pages, `0` source-refresh pages, `0` publication-date-review pages, and `0` editorial-review pages. The product-reference, competitive-context, compendium singleton, Volume 02, Volume 03, Volume 04 Funding Rate Model, and Volume 04 USDC-vs-token-margined lanes have no remaining candidate pages; protocol-reference candidates are down to `96`, rewards-referrals candidates are down to `84`, manifesto candidates are down to `40`, and the remaining volume lanes are Volume 04 (`28`), Volume 05 (`108`), and Volume 06 (`84`).
