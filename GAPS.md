...
The candidate review queue now includes the remaining `430` candidate pages after the first `368` source/prose/route/current-source/operator-boundary-reviewed pages moved to `published`. Its launch lanes are `0` final-review-ready pages, `430` operator-review pages, `0` source-refresh pages, `0` publication-date-review pages, and `0` editorial-review pages. The product-reference, competitive-context, and compendium singleton lanes have no remaining candidate pages.
