...
The candidate review queue now includes the remaining `192` candidate pages after the first `606` source/prose/route/current-source/operator-boundary-reviewed pages moved to `published`. Its launch lanes are `0` final-review-ready pages, `192` operator-review pages, `0` source-refresh pages, `0` publication-date-review pages, and `0` editorial-review pages. The product-reference, competitive-context, compendium singleton, Volume 02, Volume 03, and Volume 04 lanes have no remaining candidate pages; protocol-reference candidates are down to `95`, rewards-referrals candidates are down to `84`, manifesto candidates are down to `13`, and the remaining volume lanes are Volume 05 (`108`) and Volume 06 (`84`).
